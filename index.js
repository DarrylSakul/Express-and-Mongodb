...
app.post("/upload", fileUpload.single("file"), (req, res) => {
  ...
  if (file) {
    ...
    res.send("file failed to be uploaded");
  }
});
...
app.put("/users/:name", (req, res) => {
  if (req.body.name == "") {
    res.json({ message: "error, could not find data from request body" });
  }
  users.forEach((e) => {
  ...
  res.json({ message: "could not find specific name" });
});
...
app.delete("/users/:name", (req, res) => {
  ...
  res.json({ message: "could not find suitable name" });
});
